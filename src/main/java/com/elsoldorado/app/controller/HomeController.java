...
public class HomeController {
    ...
    public Map<String, Object> inicio() {
        return Map.of(
                ...
                "menuPredefinido", menuService.obtenerVistaPredefinida().stream().map(RestauranteMapper::toPlatoResponse).toList(),
                "platosDestacados", menuService.obtenerPlatosDestacados().stream().map(RestauranteMapper::toPlatoResponse).toList()
        );
    }
}
